...
def hr2d(N, T, t0, dt, sd, D, a, b, c, d, s, r, x1, I0, stim, blocks):
    # initialize Hindmarsh-Rose system
    x = np.zeros((N,N))
    y = np.zeros((N,N))
    z = np.zeros((N,N))
    dxdt = -1.2*np.ones((N,N)) # np.zeros((N,N))
    dydt = np.zeros((N,N))
    dzdt = np.zeros((N,N))
...
def main():
    ...
    c = 1.0
    d = 5.0
    s = 4.0 # s=1: spiking, s=4: strong accomodation
    r = 0.001 # 0.001
    x1 = -1.6
    I0 = 3.5 # 1.3
    #I1 = 0.05 # 1.3
    ...
    # run simulation
    data = hr2d(N, T, t0, dt, sd, D, a, b, c, d, s, r, x1, I0, stim, blocks)
